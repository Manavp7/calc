                                ...
                                onChange={async (e) => {
                                    ...
                                    try {
                                        ...
                                        if (data.success && data.text) {
                                            const extractedText = `[From document: ${file.name}]\n\n` + data.text;
                                            setIdeaText(extractedText);

                                            // Auto-trigger analysis after extraction
                                            const analysisRes = await fetch('/api/analyze-idea', {
                                                method: 'POST',
                                                headers: { 'Content-Type': 'application/json' },
                                                body: JSON.stringify({ ideaText: extractedText.trim() }),
                                            });
                                            const analysisData = await analysisRes.json();
                                            if (analysisData.success && analysisData.analysis) {
                                                onAnalysisComplete(analysisData.analysis);
                                            } else {
                                                setError(analysisData.error || 'Document was read but analysis failed. You can still click "Analyze My Idea".');
                                            }
                                        } else {
                                            setError(data.error || 'Failed to read file content. Please check the file is a valid PDF or DOCX.');
                                        }
                                    } catch (err) {
                                        console.error(err);
